...
class U_NeXt_v2(nn.Module):
    def __init__(self,
        ...
        self.down_sample = nn.ModuleList()
        for i in range(self.n_stages):
            self.down_sample.append(ailoc.transloc.DownSampleBlock_v2(curr_c, curr_c * 2, kernel_size))
            curr_c *= 2

        # up sampling blocks
        self.up_sample = nn.ModuleList()
        for i in range(n_stages):
            self.up_sample.append(ailoc.transloc.UpSampleBlock_v2(curr_c, curr_c // 2, kernel_size))
            curr_c //= 2
