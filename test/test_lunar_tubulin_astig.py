...
def lunar_loclearning_ckpoint():
    model_name = '../results/2024-11-08-13-56LUNAR_LL.pt'
    ...
    # analyze the experimental data
    image_path_list = ['../datasets/simu_tubulin/simu_tubulin_astig_ld_v3',  # can be a tiff file path or a folder path
                       '../datasets/simu_tubulin/simu_tubulin_astig_md_v3',
                       '../datasets/simu_tubulin/simu_tubulin_astig_hd_v3', ]
    gt_path_list = ['../datasets/simu_tubulin/simu_tubulin_astig_ld_v3/activations.csv',
                    '../datasets/simu_tubulin/simu_tubulin_astig_md_v3/activations.csv',
                    '../datasets/simu_tubulin/simu_tubulin_astig_hd_v3/activations.csv', ]
...
def lunar_synclearning():
    ...
    if calib_file is not None:
        ...
        psf_params_dict = {
            ...
            'npupil': 64,
            'psf_size': 35,
            'objstage0': -1000,  # Nominal focus position (close to sample is minus, focus on coverslip is 0), unit: nm
            # 'zemit0': 0,  # distance from the coverslip to the sample, surrounded by the training z range, unit: nm
        }
    ...
    if experiment_file is not None:
        ...
        # roi_library = ailoc.common.build_roi_library(experimental_images_photon,
        #                                              experimental_images,
        #                                              psf_params_dict,
        #                                              sampler_params_dict,
        #                                              sparse_first=True,
...
def lunar_synclearning_ckpoint():
    model_name = '../results/2024-11-06-10-36LUNAR_SL.pt'
    with open(model_name, 'rb') as f:
        lunar_model = torch.load(f)

    experiment_file = '../datasets/simu_tubulin/simu_tubulin_astig_ld_v2/1.tif'
    experimental_images = ailoc.common.read_first_size_gb_tiff(experiment_file, 2)
    ...
    # read the ground truth and calculate metrics
    gt_array = ailoc.common.read_csv_array("../datasets/simu_tubulin/simu_tubulin_astig_ld_v2/activations.csv")
...
def lunar_analyze():
    loc_model_path = '../results/2024-11-08-13-56LUNAR_LL.pt'
    image_path = '../datasets/simu_tubulin_v2/simu_tubulin_astig_ld'   # can be a tiff file path or a folder path
    save_path = '../results/' + \
