...
def deeploc_train():
    ...
    deeploc_model2 = ailoc.deeploc.DeepLoc(psf_params_dict, camera_params_dict, sampler_params_dict)
    deeploc_model2.check_training_psf()
    deeploc_model2.check_training_data()
    gt_csv = '../datasets/mismatch_data2/activations.csv'
    deeploc_model2.evaluation_dataset['data'] = ailoc.common.cpu(experimental_images)[0:2000, None, :, :]
    molecule_list_gt = sorted(ailoc.common.read_csv_array(gt_csv), key=lambda x: x[0])
    end_idx = ailoc.common.find_frame(molecule_list_gt, frame_nbr=2000)
    deeploc_model2.evaluation_dataset['molecule_list_gt'] = np.array(molecule_list_gt[:end_idx])
    file_name = '../results/' + datetime.datetime.now().strftime('%Y-%m-%d-%H') + 'DeepLoc2.pt'
    deeploc_model2.online_train(batch_size=10,
                               max_iterations=30000,
                               eval_freq=500,
                               file_name=file_name)
    # plot evaluation performance during the training
    ailoc.common.plot_train_record(deeploc_model2)
    # analyze the experimental data
    image_path = os.path.dirname(experiment_file)  # can be a tiff file path or a folder path
    save_path = '../results/' + \
                os.path.split(file_name)[-1].split('.')[0] + \
                '_' + os.path.split(image_path)[-1].split('.')[0] + '_predictions.csv'
    print(save_path)
    deeploc_analyzer = ailoc.common.SmlmDataAnalyzer(loc_model=deeploc_model2,
                                                     tiff_path=image_path,
                                                     output_path=save_path,
                                                     time_block_gb=1,
                                                     batch_size=16,
