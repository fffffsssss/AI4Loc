...
def transloc_train():
    ...
    if calib_file is not None:
        ...
        zernike_aber = np.array([2, -2, 0, 2, 2, 70, 3, -1, 20, 3, 1, 0, 4, 0, 20, 3, -3, 0, 3, 3, 0,
    ...
    transloc_model.evaluation_dataset['data'] = ailoc.common.cpu(experimental_images)[0:2000, None, :, :]
    ...
    transloc_model.evaluation_dataset['molecule_list_gt'] = np.array(molecule_list_gt[:end_idx])
    ...
    # train another deeploc model and compare the performance
    sampler_params_dict['bg_perlin'] = True
    ...
    transloc_model2 = ailoc.transloc.TransLoc(psf_params_dict, camera_params_dict, sampler_params_dict)
    transloc_model2.check_training_psf()
    transloc_model2.check_training_data()
    gt_csv = '../datasets/mismatch_data2/activations.csv'
    transloc_model2.evaluation_dataset['data'] = ailoc.common.cpu(experimental_images)[0:2000, None, :, :]
    molecule_list_gt = sorted(ailoc.common.read_csv_array(gt_csv), key=lambda x: x[0])
    end_idx = ailoc.common.find_frame(molecule_list_gt, frame_nbr=2000)
    transloc_model2.evaluation_dataset['molecule_list_gt'] = np.array(molecule_list_gt[:end_idx])
    file_name = '../results/' + datetime.datetime.now().strftime('%Y-%m-%d-%H') + 'TransLoc2.pt'
    transloc_model2.online_train(batch_size=10,
                               max_iterations=30000,
                               eval_freq=500,
                               file_name=file_name)
    # plot evaluation performance during the training
    ailoc.common.plot_train_record(transloc_model2)
    # analyze the experimental data
    image_path = os.path.dirname(experiment_file)  # can be a tiff file path or a folder path
    save_path = '../results/' + \
                os.path.split(file_name)[-1].split('.')[0] + \
                '_' + os.path.split(image_path)[-1].split('.')[0] + '_predictions.csv'
    print(save_path)
    transloc_analyzer = ailoc.common.SmlmDataAnalyzer(loc_model=transloc_model2,
                                                     tiff_path=image_path,
                                                     output_path=save_path,
                                                     time_block_gb=1,
                                                     batch_size=16,
...
def deeploc_ckpoint_train():
    model_name = '../results/2023-10-21-11TransLoc.pt'
